...
function checkStatus() {
  ...
  const hDot = document.getElementById('hApiDot');
  dot.className = 'status-dot';
  lbl.textContent = 'Connecting…';
  if (hDot) hDot.className = 'status-dot';

  fetch(API)
    .then(r => {
      if (r.ok) {
        dot.classList.add('on'); lbl.textContent = 'API Connected';
        if (hDot) hDot.classList.add('on');
      } else throw new Error();
    })
    .catch(() => {
      dot.classList.add('off'); lbl.textContent = 'Offline';
      if (hDot) hDot.classList.add('off');
    });
}
...
function switchTab(name, el) {
  ...
  const titles = { home:'Home', dashboard:'Dashboard', catalog:'Product Catalog', add:'Add Product', analytics:'Analytics', settings:'Settings' };
...
function updateHeroStats() {
  ...
  const hCats  = document.getElementById('hCats');
  if (hTotal) hTotal.textContent = allProducts.length;
  if (hCats)  hCats.textContent  = new Set(allProducts.map(p => p.category)).size;
}
